<template>
  <div
    :class="
      time === 'night'
        ? 'userInfo nightBgc nightColor'
        : 'userInfo morningBgc morningColor'
    "
  >
    <Header></Header>
    <div class="userInfo__container animate__animated animate__fadeIn">
      <ul class="userInfo__ul" v-if="showRealUserList">
        <li
          class="userInfo__li"
          v-for="(item, index) in userList"
          :key="index"
          @click="gotoPersonalPage(index)"
        >
          <div
            class="bannerOfUser"
            :style="
              item.image_coverpage == ''
                ? { backgroundImage: `url(${ifUserImageCoverpageIsNull})` }
                : { backgroundImage: `url(${url + item.image_coverpage})` }
            "
          >
            <div class="avatarBackground">
              <div
                class="rotateContainer"
                v-if="item.image_avatar.indexOf('POAP') > -1 ? true : false"
              ></div>
              <img :src="url + item.image_avatar" alt="" class="userAvatar" />
            </div>
          </div>
          <div class="center__container">
            <h1 class="userName">
              {{ item.username == "" ? index : item.username }}
            </h1>
            <div class="nftAndMuseum">
              <div class="nftAndMuseumItems">
                <div class="nftAndMuseumItems__left">
                  <h1 class="nft">NFT</h1>
                  <h2 class="nftCounts">{{ item.count_nft }}</h2>
                </div>
                <div class="nftAndMuseumItems__right">
                  <h1 class="museum">Museum</h1>
                  <h2 class="museumCounts">{{ item.count_museum }}</h2>
                </div>
                <div class="nftAndMuseumItems__center">
                  <h1 class="nft">POAP</h1>
                  <h2 class="nftCounts">{{ item.count_poap }}</h2>
                </div>
              </div>
            </div>
          </div>
      ...
      <el-skeleton
        ...
        class="userInfoSke"
        :throttle="100"
        v-if="showSkeletonOfUsers"
      >
        <template slot="template">
          <ul class="userInfo__ul__ske">
            <li v-for="index in 21" :key="index" class="userInfo__li__ske">
              <div class="bannerOfUser__ske">
                <el-skeleton-item variant="p" class="bannerOfUserSke" />
                <div class="avatarBackground__ske">
                  <el-skeleton-item variant="p" class="avatarOfUserSke" />
                </div>
                <div class="center__container__ske">
                  <el-skeleton-item variant="p" class="userName" />
                  <div class="nftAndMuseum">
                    <div class="nftAndMuseumItems">
                      <div class="nftAndMuseumItems__left">
                        <el-skeleton-item variant="p" class="nft" />
                        <el-skeleton-item variant="p" class="nftCounts" />
                      </div>
                      <div class="nftAndMuseumItems__right">
                        <el-skeleton-item variant="p" class="museum" />
                        <el-skeleton-item variant="p" class="museumCounts" />
                      </div>
                      <div class="nftAndMuseumItems__center">
                        <el-skeleton-item variant="p" class="nft" />
                        <el-skeleton-item variant="p" class="nftCounts" />
                ...
                <div class="footerDiscription">
                  <el-skeleton-item variant="p" class="footDes" />
                </div>
              </div>
            </li>
          </ul>
...
export default {
  ...
  data() {
    return {
      url: window.urls,
      userList: "",
      ifUserImageCoverpageIsNull: require("../assets/personal/nav.png"),
      showRealUserList: false,
      showSkeletonOfUsers: true,
      loading: true,
    };
  },
  methods: {
    gotoPersonalPage(index) {
  ...
  mounted() {
    this.$axios.post(this.url + "get_all_user_test").then((res) => {
      this.userList = res.data;
      this.loading = false;
      this.showRealUserList = true;
      this.showSkeletonOfUsers = false;
    });
  },
};
</script>
...
@import "~@/assets/stylesheet/UserInfo.less";
</style>
